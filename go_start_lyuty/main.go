...
// 8.2 Горутины
fmt.Println("8.2 Горутины")

{
	fmt.Println(utils.GetValue())

	// utils.Out(0, 5)
	// utils.Out(6, 10)
	// Функции выполняются друг за другом
}
...
// 8.3 Каналы
fmt.Println("8.3 Каналы")

{
// Горутины выполняются независимо друг от друга и не знают, когда закончилась другая горутина. 
// Это приводит к тому, что, например, функция main() может завершиться до того, как завершится любая запущенная горутина.
...
// Чтобы использовать канал, нам сначала нужно создать его с помощью функции make():
	...
	// Мы можете отправить данные в канал, используя следующий синтаксис:
...
{
	// Создать канал, передать его в качестве аргумента функции test(), 
	// вызвать ее как горутину и получить значение из канала, присвоив его переменной res
...
{
	fmt.Println("Теперь мы можем использовать наш канал и переписать предыдущий пример без time.Sleep() в main()")
	// Мы определяем канал типа bool и передаем его нашей функции out() в качестве аргумента. 
	// После того как функция выполнит свою задачу, мы отправим в канал значение true, которое будет получено в main().
	// Теперь main() ожидает получения данных из канала, заставляя программу ждать завершения выполнения горутин.

	// Операция получения блокирует код до тех пор, пока не будут отправлены какие-либо данные операцией отправки. 
	// Если данные не будут получены, возникнет deadlock, блокирующий выполнение кода.


	ch := make(chan bool)

	go utils.OutCh(0, 5, ch)
	go utils.OutCh(6, 10, ch)
...
{
	// Наша программа должна вычислить и вывести сумму всех четных чисел в заданном диапазоне плюс сумму их квадратов и вывести результат:
	// output = evenSum+squareSum

	// Мы будем использовать две горутины: одну для вычисления evenSum, а другую для вычисления squareSum.
	// Мы получим данные с помощью каналов в main(), затем вычислим и выведем окончательную сумму.

	evenCh := make(chan int)
	sqCh := make(chan int)

	go utils.EvenSum(0, 100, evenCh)
	go utils.SquareSum(0, 100, sqCh)

	fmt.Println(<-evenCh + <-sqCh)
}

// 8.4 Выбор (Select)
fmt.Println("8.4 Выбор (Select)")

{
	// Оператор select используется для ожидания нескольких операций канала.
	// Синтаксис похож на switch, за исключением того, что 
	// каждый из операторов case будет операцией канала.
}

{
	...
	// Представьте себе программу, которой необходимо выполнять некоторый код всякий раз, 
	// когда завершается одна из многопоточных операций - этого можно добиться с помощью select.
}

{
	fmt.Println("Программа 'Загрузчик'")
	// Загрузчик
    ...
    go utils.Download(s1, ch1)
    go utils.Download(s2, ch2)
    go utils.Download(s3, ch3)
...

{
	...
	// Цикл for использует select для проверки того, какой канал получил данные. Если ни один из них не готов, 
	// будет выполнен случай default, который будет ждать 50 мс.
	...
	// Случай default полезен для предотвращения возникновения тупиковых ситуаций (deadlocks) - без него select будет ждать канал вечно, 
	// что приведет к аварийному завершению программы, если ни один из каналов не получит данных.
}
}
